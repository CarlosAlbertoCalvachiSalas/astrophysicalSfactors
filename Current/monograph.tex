...
\begin{equation}\label{micro_threeBody_wavefunction}
...
\begin{equation}\label{micro_threeBody_splitting}
...
\begin{equation}\label{micro_threeBody_psi3}
...
\begin{equation}\label{micro_threeBody_psi2}
	\Psi^{(2)}(X, Y) =  \sum_{\alpha} {f_\alpha (\rho) \psi_{l_xj_x}(X) \psi_{l_yj_y}(Y) }, 
\end{equation}

where $\alpha$ is an index running through all the values of $l_x$, $l_y$, $j_x$ and $j_y$. Also, like it happens with the expression of $chi_{K, \nu}$ of equation  \ref{micro_threeBody_chiAsymptotics}, the functions $\psi$ obey asymptotics. In this concrete case, the extreme behavior is given as: 

\begin{equation}\label{micro_threeBody_psi2_asymptotic}
...
Additionally, the functions $f_\alpha$ are associated with the spectroscopic factors for each contribution to the expansion of equation \ref{micro_threeBody_psi2}. \\
...
\begin{equation}\label{micro_noCoreShell_basis}
...
\begin{equation}\label{micro_noCoreShell_continuum}
...
\begin{equation}\label{micro_noCoreShell_antisymmetrization}
...
\begin{equation}\label{eq:micro_hartreeFock_potential_waveFunction}
	U(\vec r) = \sigma_{\alpha} \int \phi_\alpha^{*} (\vec r')  V(\hat r, \hat r') \phi_\alpha(\vec r') d \vec r ',
\end{equation}
